...
//+++++++++++ Timer variables ++++++++++
unsigned long timer1DurationFar = 736; //time(ms) from start when gate will open (FAR)
unsigned long timer2DurationFar = 1597; //time(ms) gate will remain open (FAR)
unsigned long timer1DurationNear = 0; //time(ms) from start the gate will open (NEAR)
unsigned long timer2DurationNear = 1148; //time(ms) gate will remain open (NEAR)
//++++++++++++++++++++++++++++++++++++++
...
void loop() {
  ...
  if (OPEN) {
    LCD.print("  OPEN  ");
    analogWrite(mot2DirAPin, motorUpPWMCounts);
    digitalWrite(mot2DirBPin, LOW);
    if (TIMING1_OPEN) {
      if (timerNear) {
  ...
  if (CLOSE) {
    LCD.print("  CLOSE ");
    digitalWrite(mot2DirAPin, LOW);
    analogWrite(mot2DirBPin, motorDownPWMCounts);
  }
